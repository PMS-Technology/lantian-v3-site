...
export function StickyFooter() {
  ...
  const linkColumns = [
    [
      { label: "ServerNav", href: "https://nav.lantian.pro/" },
      { label: "节点状态", href: "https://status.lantian.pro/" },
      { label: "面板控制台", href: "https://panel.lantian.pro/" },
    ],
    [
      { label: "QQ 群", href: "https://qm.qq.com/q/BCUiiIQr5Y" },
      { label: "ByteFRP", href: "https://frp.bytedesktop.com/" },
      { label: "最新新闻", href: "https://next.lantian.pro/news" },
    ],
  ]
          ...
          >
            ...
            >
              {linkColumns.map((column, columnIndex) => (
                <ul className="space-y-2" key={columnIndex}>
                  {column.map((item) => (
                    <li key={item.label}>
                      <a
                        href={item.href}
            ...
            <motion.h2
              className="pointer-events-none absolute bottom-0 left-0 translate-y-1/3 sm:text-[192px] text-[80px] font-bold select-none"
              style={{ color: "#121113" }}
